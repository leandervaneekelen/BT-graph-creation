...
def get_indices(nb_elements, split=0.4):
    ...
    assert len(rand) > 3
    test_val_size = max(
        int(nb_elements * split / 2), 1
    )  # ensure that at least 1 element is present
...
def get_splits_dict(patient_id_class_pair, split=0.4):
    ...
    """
    classes, patients_per_class = np.unique(
        np.array([i["class"] for i in patient_id_class_pair]), return_counts=True
    )
    class_patients = {
        c: [i["patient_id"] for i in patient_id_class_pair if i["class"] == c]
        for c in classes
    }
    ...
    splits_per_class = {
        c: [
            [patients[i] for i in subset]
            for subset in get_indices(len(patients), split=split)
        ]
        for c, patients in class_patients.items()
    }
...
def get_file_class_pairs_per_split(data, patients_splits_dict, endpoint):
    ...
    for c, patient_id_tup in patients_splits_dict.items():
        data_subsets = [
            {pat_id: data[pat_id] for pat_id in pat_id_list}
            for pat_id_list in patient_id_tup
        ]
...
def get_xml_tree(file_class_list, dataset_name, endpoint):
    ...
    cxl = ET.SubElement(
        xml_tree,
        dataset_name,
        {"counts": str(len(file_class_list)), "endpoint": endpoint},
    )

    # add the elements
    for dic in file_class_list:
        element_cxl = ET.SubElement(
            cxl, "print", {"file": dic["file"], "class": dic["class"]}
        )
...
def json_to_cxl(json_path, output_folder, endpoint, dataset_name, split=0.4, seed=42):
    ...
    train_split, val_split, test_split = get_file_class_pairs_per_split(
        data, patients_splits_dict, endpoint
    )
    ...
    for filename, tree in zip(
        ["train.cxl", "val.cxl", "test.cxl"], [train_cxl, valid_cxl, test_cxl]
    ):
        ET.ElementTree(tree).write(
            os.path.join(output_folder, filename), pretty_print=True
        )
