...
class HotspotsCsvToXml:
    def __init__(
        self,
        hotspot_csvs_path: str,
        output_path: str,
        spacing: int = 0.242797397769517,
        hotspot_size: int = 0.785,
        top: int = 10,
    ):
    ...
    def process_files(self):
        ...
        for csv_file in self.csv_files:
            ...
            output_file = os.path.join(
                self.output_path, f"{file_id}_autom_hotspots.xml"
            )
            hotspots = pd.read_csv(csv_file, index_col="hotspot_id")[["x", "y"]]
            hotspots = hotspots.head(self.top)
            xml_tree = self.coord_list_to_xml(
                hotspots.to_dict(orient="index"), pixel_shift
            )
    ...
    def coord_list_to_xml(id_coord, pixel_shift):
        ...
        for annotation_id, centre_coord in id_coord.items():
            attrib = {
                "Name": "Annotation {}".format(annotation_id.split("_")[-1]),
                "PartOfGroup": "hotspot",
                "Color": "#73d216",
                "Type": "Rectangle",
            }
