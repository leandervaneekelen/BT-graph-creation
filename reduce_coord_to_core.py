...
def in_circle(center_x, center_y, radius, coord):
    mask = [
        ((center_x - row[0]) ** 2 + (center_y - row[1]) ** 2) <= radius**2
        for row in coord
    ]
...
def parse_tma_coord_csv(coord_csvs):
    ...
    for file_path in coord_csvs:
        if os.path.isfile(file_path):
            ...
            df.insert(
                0,
                "TMA_filename",
                re.search(r"Coordinates_(.*).csv", file_path).group(1),
            )
    ...
    all_core_coord = all_core_coord[
        all_core_coord.columns.drop(list(all_core_coord.filter(regex="microns")))
    ]
    return all_core_coord


def create_core_only_txt_files(
    ...
    for file_id, file_path in txt_files_to_process.items():
        ...
        for label in ["lymphocytes", "tumorbuds"]:
            ...
            if os.path.isfile(f"{file_path}_coordinates_{label}.txt"):
                ...
                coord_cores = cores_df.apply(
                    lambda row: in_circle(
                        row["Centroid X (pixels)"],
                        row["Centroid Y (pixels)"],
                        row["Radius (pixels)"],
                        coord,
                    ),
                    axis=1,
                ...
                coord_cores = {
                    core_id: c
                    for core_id, c in zip(cores_df["Core Unique ID"], coord_cores)
                }
                for core_id, coords in coord_cores.items():
                    output_txt_file = os.path.join(
                        txt_output, f"{file_id}_{core_id}_coordinates_{label}.txt"
                    )
        ...
        for index, row in cores_df.iterrows():
            ...
            output_txt_file = os.path.join(
                txt_output, f'{file_id}_{row["Core Unique ID"]}_coordinates_hotspot.txt'
            )
            if not os.path.isfile(output_txt_file) or overwrite:
