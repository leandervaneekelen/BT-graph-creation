        ...
        if self.full_coord:
            ...
            if hotspots is None:
                ...
                data = [
                    {
                        "hotspot": h,
                        "lymphocytes": self.read_txt_file(
                            "lymphocytes", id=f"_hotspot{i}"
                        ),
        ...
        for coord_dict in self.data:
            ...
            for group, coordinates in coord_dict.items():
                ...
                if coordinates is not None and len(coordinates) > 0:
                    # make the group
                    xml_group = ET.SubElement(
                        xml_annotation_groups,
                        "Group",
                        attrib={
                            "Name": group,
                            "PartOfGroup": "None",
                    ...
                    for i, line in enumerate(coordinates):
                        ...
                        if annotation_type == "Dot":
                            xml_annotation = ET.SubElement(
                                xml_annotations, "Annotation", attrib
                            )
                            xml_coordinates = ET.SubElement(
                                xml_annotation, "Coordinates"
                            )
                            coor_attrib = {
                                "Order": "0",
                                "X": str(line[0]),
                            ...
                            xml_coordinate = ET.SubElement(
                                xml_coordinates, "Coordinate", attrib=coor_attrib
                            )
                        else:
                            xml_annotation = ET.SubElement(
                                xml_annotations, "Annotation", attrib
                            ...
                            coor = [
                                (line[i], line[i + 1]) for i in range(0, len(line), 2)
                            ]
...
def create_asap_xml(
    coordinates_txt_files_folder: str,
    output_folder: str,
    separator_id: str = "_coord",
    full: bool = False,
):
    ...
    files_to_process = list(
        set(
            [
                re.search(repr(f"(.*){separator_id}")[1:-1], f).group(1)
                for f in all_files
            ]
        )
    )
